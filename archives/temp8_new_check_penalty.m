function temp8_new_check_penalty()
% the new activeness check criteria is tried out
% experimenting on new updating scheme for penalty parameter 
% can be benchmarked against temp5_inequ_con() 
% To test: 
% min f = x^2 - y^2;
% s.t. -1 < y < 0
close all
...
while k < max_iter
    ...
    subspace_size = length(xk) + length(find(c(m_eq+1 : m_con)<0));
...
%     m_active = length(row_keep);                % num of active inequ cons
    ...
    if num_inactive > 0           % if inactive exists, inside feasible region       
        ...
        for i=1:num_inactive
            c_h{idx_inactive(i)} = zeros(size(c_h{idx_inactive(i)}));
        end
