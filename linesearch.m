function [alpha, fnew, feval, dfeval] = linesearch(merit_fun, xk, lambda, dx, mu_1,...
...
assert(size(xk,1) + size(lambda,1) == size(dx,1),...
...
alpha = alpha_low - 0.5*df_low*(dalpha^2)/(f_hi - f_low - df_low*dalpha);
min_alpha = min(alpha_hi, alpha_low);
max_alpha = max(alpha_hi, alpha_low);

if (max_alpha < alpha) && (alpha < min_apha);
    error('Error in quadraticStep: step out of range');
end

if (alpha - min_alpha) < 1e-2*(max_alpha -min_alpha);
